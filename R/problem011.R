...
# In the 20×20 grid below, four numbers along a diagonal line have been surrounded by brackets.
...
problem011 <- function(){
  ...
  X <- gsub("\\s+"," ",raw_string) %>% # remove newlines but keep single spaces
    strsplit(" ") %>% # split into an array on spaces
    sapply(as.integer) %>% # convert elements to ints
    .[,1] %>% # extract to vector
    matrix(nrow=20, ncol=20, byrow=TRUE) # turn from 1x400 vector into 20x20 matrix by row

  # From every spot in the grid X[row=i,col=j], we'll test in four direction:
  #   - Across: i,j+0:3
  #   - Down: i+0:3,j
  #   - Up and right: i-0:3,j+0:3
  #   - Down and right: i+0:3, j+0:3
