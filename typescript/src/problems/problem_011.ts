...
export function main(): number {
    ...
    for (let row = 0; row < n_rows; row++) {
        for (let col = 0; col < n_cols; col++) {
            const right = get(A, row, col) * get(A, row, col + 1) * get(A, row, col + 2) * get(A, row, col + 3)
            const down = get(A, row, col) * get(A, row + 1, col) * get(A, row + 2, col) * get(A, row + 3, col)
            const up_right = get(A, row, col) * get(A, row - 1, col + 1) * get(A, row - 2, col + 2) * get(A, row - 3, col + 3)
            const down_right = get(A, row, col) * get(A, row + 1, col + 1) * get(A, row + 2, col + 2) * get(A, row + 3, col + 3)
